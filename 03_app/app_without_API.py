...
checkpoint_path = os.path.join(salivary_gland, "00_epochs", "save_at_23.keras")
...
<p align="center">
  <img src="https://imgtr.ee/images/2023/07/14/b1deb27bdd471bb2b8fc5374d9d9cca5.png" width="100%" alt="Cytology Image">
  <br>
</p>
...
page = st.sidebar.radio('Select a Page', ['Home', 'Diagnosis', 'Chapter I: Cytology', 'Chapter II: Data', 'Chapter III: Neural Networks'])

with st.spinner('Loading page...'):
    if page == 'Home':
        st.title('Cytology Codex')
        st.header('The Digital Tome of Cytology I: Deep Learning & Neural Networks Transmutation')
        st.markdown('''
        <p align="center">
          <img src="https://cdn.discordapp.com/attachments/1129416761922568212/1130371846781534289/isi.mube_a_closed_spellbook_tome_that_speaks_about_Python_Neura_fcd7bb99-c40d-429c-a8f3-9f2bbdad3d7e.png" width="36%" alt="Cytology Image">
          <br>
    ...
    if page == 'Diagnosis':
        # title
        st.title("Diagnosis and Report")

        # classes dictionary
        class_salivary = {i: name.replace('_', ' ').title() for i, name in enumerate(['acinar_carcinoma', 'adenoid_cystic_carcinoma', 'benign', 'pleomorphic_adenoma', 'warthin_tumor'])}

        # organ sample and upload image
        organ_sample = st.selectbox('Select the organ sample:', ['Salivary Gland', 'Sample 2', 'Sample 3'])
        ...
        if uploaded_file is not None:
            ...
            with st.spinner('Predicting...'):
                predictions = salivary_model.predict(img_array)
                class_idx = np.argmax(predictions[0])
                class_prob = predictions[0][class_idx]
                class_sg = class_salivary[class_idx] 
            ...
            with col2:
                col2.markdown(f"<h2 style='text-align: center; color: green; display: inline-block;'>Diagnosis: {class_sg}</h2>", unsafe_allow_html=True)
                col2.markdown(f"#### Confidence: **{class_prob*100:.2f}%**")
                col2.markdown("### Cytology Consultant", unsafe_allow_html=True)
                    ...
                    with st.spinner('Consulting with AI...'):
                        response = openai.ChatCompletion.create(
                            ...
                            messages=[
                                ...
                                {"role": "user", "content": f"Which are the key cytological features and malignancy risk for {class_sg} in the {organ_sample}?"}
                            ]
                        )
                    col2.markdown(f"Cytology Consultant: {response['choices'][0]['message']['content']}")
                    col2.markdown("###### AI powered by Chat GPT-3", unsafe_allow_html=True)

            with col1:
                # button queries
                button_queries = {
                    "Useful IHQ": f"What are the most frequently used immunohistochemical markers for {class_sg}?",
                    "Differential Diagnosis": f"What are the differential diagnoses for {class_sg}?"
                }
                # button text
                for button_text, query in button_queries.items():
                    if st.button(button_text):
                        ...
                        col1.markdown(f"Cytology Consultant: {response['choices'][0]['message']['content']}")

                # empty box for the user
    ...
    elif page == 'Chapter III: Neural Networks':
        st.markdown('''
